<div>
    <main x-data="{createForm: @entangle('createForm')}" class="relative z-0 flex-1 min-h-screen overflow-y-auto focus:outline-none">
        ...
        <x-modals.dialog wire:model="createForm" maxWidth="2xl">
            <x-slot name="title">Információ létrehozása</x-slot>

            <x-slot name="content">
                <div id="editor">Az információ helye!</div>
            </x-slot>
            <x-slot name="footer">
                {{-- <button --}}
                    {{-- type="submit" --}}
        ...
        <div class="block mt-8 sm:block">
            <div x-data="{confirmingItemDeletion: @entangle('confirmingItemDeletion')}" class="inline-block min-w-full align-middle border-b border-gray-200">
                ...
                @foreach($this->doctorGroup as $doctor => $consultations)
                    <h2 class="mt-5 ml-3 font-bold">{{ $this->getDoctorName($doctor) }}</h2>
                    <table class="min-w-full">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @foreach($consultations as $day => $consultation)
                                <tr>
                                    ...
                                    <td class="px-6 py-3 text-sm font-medium text-center text-gray-500">
                                        <div class="flex items-center justify-center space-x-2">
                                            <div class="flex flex-col items-center flex-shrink-0 -space-x-1">
                                                <a href="{{ route('admin.consultations.show', [$doctor, $day]) }}" class="font-bold truncate text-md hover:text-gray-600">
                                                    <span>{{ $day }}</span>
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div x-data="{itemDropdown: false}" class="relative flex items-center justify-end">
                                            ...
                                            <div x-show="itemDropdown === true" class="absolute top-0 z-10 w-48 mx-3 mt-1 origin-top-right bg-white divide-y divide-gray-200 rounded-md shadow-lg right-7 ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="project-options-menu-0-button" tabindex="-1">
                                                <div class="py-1" role="none">
                                                    ...
                                                    <a href="{{ route('admin.consultations.show', [$doctor, $day]) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 group" role="menuitem" tabindex="-1" id="project-options-menu-0-item-0">
